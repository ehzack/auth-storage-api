...
router.post("/logout", async (req, res, next) => {
  ...
  const { refresh_token, mac } = value;

  // delete refresh token passed in data

  console.log(refresh_token, mac);
...
async function main(email, language = "fr", token) {
  try {
    let id = await User.updateSecretTokenExpires(email, token);
    var subject = "Reset Password";
    if (language == "en" || language == "fr" || language == "es") {
      var html = await require(`./mail/activate_template/index`)(
        token,
        language
      );
      var i18n = require(`./mail/activate_template/i18n/${language}.json`);
      subject = i18n.object;
    } else {
      var html = await require(`./mail/activate_template/index`)(token, "en");
    }
    ...
    auth_functions.sendMail(msg);
  } catch (error) {
    console.log(error);
  }
}

async function mainResetPassword(email, language = "fr", token) {
  try {
    var subject = "Account activation";

    if (language == "en" || language == "fr" || language == "es") {
      var html = await require(`./mail/formForgotpassword/index`)(
        token,
        language
      );
      var i18n = require(`./mail/activate_template/i18n/${language}.json`);
      ...
      var html = await require(`./mail/formForgotpassword/index`)(token);
    }
...
router.post("/init-activate-account", async (req, res, next) => {
  ...
  console.log(result);
  if (result == false) {
    main(email, language, generatedToken);
    res.json({ status: 0 });
  } else if (result == true) {
    res.json({ status: 1 });
  } else {
...
router.post("/validateAccount", async (req, res, next) => {
  try {
    ...
    if (!error) {
      ...
      let result = await User.activateAccount(email, token);
...
router.post("/reset-password", async (req, res, next) => {
  ...
  let response = await User.update_secret_token_forgot_psw(email, token);
...
router.post("/forgot-password", async (req, res, next) => {
  ...
  if (user == true) {
    var generatedToken = phoneToken(4, { type: "number" });
    var response = await User.insertCode_token_forgot_psw(
      email,
      generatedToken
    );
    if (response == 1) {
      mainResetPassword(email, language, generatedToken);
      res.json({ status: 1 });
    } else {
