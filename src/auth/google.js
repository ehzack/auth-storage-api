...
passport.use(
  new GoogleStrategy(
    ...
    async function (request, accessToken, refreshToken, profile, cb) {
      ...
      console.log({ data_params: request.data_params });
      const query = `
  query (
    $profile_id: String!
  ) {
...
router.get(
  ...
  function (req, res, next) {
    console.log({ aff_uuid: req.query.aff_uuid });
    req.data_params = { aff_uuid: req.query.aff_uuid };
    next();
  },
  passport.authenticate("google", {
  ...
  async function (req, res) {
    ...
    if (PROVIDERS_SUCCESS_REDIRECT.indexOf("?") > 1) {
      callback_url = `${PROVIDERS_SUCCESS_REDIRECT}&type=auth&refresh_token=${refresh_token}`;
    } else {
      callback_url = `${PROVIDERS_SUCCESS_REDIRECT}?type=auth&refresh_token=${refresh_token}`;
    }
